...
export default function AthleteSidebar({
    ...
    return (
        <>
            ...
            <aside className={`fixed top-0 left-0 h-screen w-60 bg-white border-r border-gray-200/80 flex flex-col z-50 transition-transform duration-300 md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                        {tenant.logoUrl ? (
                            <img
                                src={tenant.logoUrl}
                                alt={tenant.name}
                                className="w-9 h-9 rounded-xl object-contain flex-shrink-0 border border-gray-100 shadow-sm"
                            />
                        ) : (
                            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-red-500 to-red-700 flex items-center justify-center flex-shrink-0 shadow-sm">
                                <span className="text-white font-black text-sm">{(tenant.name || 'A').charAt(0)}</span>
                            </div>
                        )}
                        <div className="flex-1 min-w-0">
                            <h2 className="text-sm font-bold text-gray-900 truncate leading-tight">{tenant.name}</h2>
                            <p className="text-[10px] text-gray-400 font-medium mt-0.5">Athlete Portal</p>
                        </div>
                    </div>
                </div>
