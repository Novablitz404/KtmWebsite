...
import { Home, CalendarCheck, User, LayoutDashboard, Users, CalendarDays, Bell, Calendar, Settings } from 'lucide-react'
...
export default function BottomTabBar({ role, userId }: BottomTabBarProps) {
    ...
    if (role === 'ATHLETE' || role === null) {
        const homeActive = isActive(['/athlete/dashboard'])
        ...
        return (
            <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-200 pb-[env(safe-area-inset-bottom)]">
                <div className="flex items-center justify-around h-16 relative">
                    ...
                    {/* Alerts */}
                    <Link
                        href="/athlete/dashboard?tab=alerts"
                    ...
                    <Link
                        href="/settings"
                        className={`flex flex-col items-center justify-center flex-1 h-full space-y-1 ${settingsActive ? 'text-indigo-600' : 'text-gray-400'}`}
                    >
    ...
    if (role === 'CLUB_MASTER' || role === 'ASSISTANT_CLUB_MASTER') {
        const tabs = [
            ...
            { href: '/settings', icon: <Settings size={22} />, label: 'Settings', pattern: /^\/settings/ },
        ]
    ...
    return (
        <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-200 pb-[env(safe-area-inset-bottom)]">
            <div className="flex items-center justify-around h-14">
                ...
                <Link href="/settings" className={`flex flex-col items-center justify-center flex-1 h-full space-y-0.5 ${pathname === '/settings' ? 'text-red-600' : 'text-gray-400'}`}>
                    <Settings size={22} />
                    <span className="text-[10px] font-medium">Settings</span>
                </Link>
            </div>
